...
export default function BuildDetailsPage({ params }: PageProps) {
  ...
  const fetchBuildDetails = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8086/api/builds/${buildId}`,
        {
          headers: token
            ? {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `ws://localhost:8085/ws?clientId=${clientId}&buildId=${buildId}`,
    );
    ...
    <div className="container mx-auto py-10">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <Badge
            className={`${getStatusColor(build.status)} text-white px-4 py-2`}
          >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Real-time build output and logs</CardDescription>
        </CardHeader>
        <CardContent>
